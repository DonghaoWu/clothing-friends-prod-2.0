...
const createCollectionAndDocsInFirestore = async (collectionKeyToCreate, arrayData) => {

    const newArrayData = arrayData.map(({ title, items }) => {
        return { title, items };
    });
...
const executeFunc = async () => {
    try {
        await createCollectionAndDocsInFirestore('collections', SHOP_DATA);
        console.log('Success.');
        console.log('Exit node now.')
        process.exit()
    } catch (error) {
        console.log(error);
    }
